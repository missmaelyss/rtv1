...
/*   Updated: 2017/06/02 17:18:06 by ele-cren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (ft_strequ(env->parse.split[0], "position") == 1)
	{
		...
		while (env->parse.split[i])
		{
			if (ft_strequ(env->parse.split[i], "x") && (i + 2) \
										<= env->parse.tablen)
				env->light.pos_x = ft_atoi(env->parse.split[i + 2]);
			else if (ft_strequ(env->parse.split[i], "y") && (i + 2) \
										<= env->parse.tablen)
				env->light.pos_y = ft_atoi(env->parse.split[i + 2]);
			else if (ft_strequ(env->parse.split[i], "z") && (i + 2) \
										<= env->parse.tablen)
				env->light.pos_z = ft_atoi(env->parse.split[i + 2]);
			i++;
		}
	}
}
