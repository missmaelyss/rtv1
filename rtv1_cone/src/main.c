...
t_vct3d rotation_matricielle(t_vct3d dir, double angle, int mod)
{
	t_vct3d ret;
	...
	if (mod == 1)
	{
		ret.x = cos(angle * (M_PI / 180))*dir.x - sin(angle * (M_PI / 180))*dir.y;
		ret.y = sin(angle * (M_PI / 180))*dir.x + cos(angle * (M_PI / 180))*dir.y;
		ret.z = dir.z;
	}	
	else if (mod == 2)
	{
		ret.x = cos(angle * (M_PI / 180))*dir.x + sin(angle * (M_PI / 180) )*dir.z;
		ret.y = dir.y;
		ret.z = -sin(angle * (M_PI / 180))*dir.x + cos(angle* (M_PI / 180))*dir.z;
	}
	else if (mod == 3)
	{
		ret.x = dir.x;
		ret.y = cos(angle * (M_PI / 180))*dir.y - sin(angle * (M_PI / 180))*dir.z;
		ret.z = sin(angle * (M_PI / 180))*dir.y + cos(angle * (M_PI / 180))*dir.z;
	}
	return (ret);
}
...
int		main(void)
{
	...
	position.y = 0;
	cone.directeur = rotation_matricielle(cone.directeur, angles.z, 1);
	cone.directeur = rotation_matricielle(cone.directeur, angles.y, 2);
	cone.directeur = (cone.directeur.x != 1 && cone.directeur.x != -1) ? rotation_matricielle(cone.directeur, angles.x, 3) : rotation_matricielle(cone.directeur, angles.x, 1);
